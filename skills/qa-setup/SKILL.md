---
name: qa-setup
description: Install a11y-cli and dq-nbomber CLI on the developer's machine. The democratize-quality MCP server is bundled in the plugin and activated automatically — no manual registration needed. Run once per developer machine before using any other skill.
...
**Only install the two tools listed below.** Do not install other packages, modify system configuration, or run any command not explicitly listed in this skill. If an installation fails in an unexpected way, stop and report the error to the user rather than attempting alternative approaches.
...
> **DQ MCP server** is bundled in the dq-awesomeqa plugin via `.mcp.json` at the repo root. It is activated automatically when the plugin is installed — no `claude mcp add` command is needed and no "disabled" approval prompt will appear.
...
If `democratize-quality` is not listed in `claude mcp list`, tell the user:
> "The democratize-quality MCP server is not active. It is bundled in the dq-awesomeqa plugin — make sure the plugin is installed in this project (`claude plugin list`) and reload Claude Code if needed."
...
**DQ MCP server** — No installation needed. Tell the user:
> "The democratize-quality MCP server is bundled in the dq-awesomeqa plugin. It was activated automatically when the plugin was installed and is listed in `claude mcp list`. No manual registration required."

If `claude mcp list` did not show `democratize-quality` in Step 1, prompt the user to check that the plugin is installed and to reload Claude Code.
...
> - ✅ democratize-quality MCP server — active via plugin (bundled in dq-awesomeqa)
>
...
| `democratize-quality` not in `claude mcp list` | Tell user to verify the dq-awesomeqa plugin is installed (`claude plugin list`) and reload Claude Code |
